...
MODEL_CLASSES = {
    ...
    't5':(AutoConfig, T5ForConditionalGeneration, RobertaTokenizer), # codet5's tokenizer is roberta's
...
def eval_line_completion(args, model, tokenizer, file_type='test'):
    ...
    if args.langs == "python":
        # break_ids = [tokenizer.sep_token_id, tokenizer.convert_tokens_to_ids('\n')]
...
def main():
    ...
    # use FileHandler output to file
    ...
    if args.eval_line:
        if args.mode == 'victim_sample_java':
